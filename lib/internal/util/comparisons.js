...
function handleCycles(val1, val2, mode, keys1, keys2, memos, iterationType) {
  ...
  if (originalSize !== set.size - 2) {
    // With no documentation on "handleCycles", it is hard to tell what the return value here is supposed to mean,
    // but I think this might be wrong since the size can increase by 1 in some cases while the obejcts are deeply equal and one of them is already in the set.
